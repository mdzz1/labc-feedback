(function($w) {
  ...
  var _CLASSNAME_ = '_labc_feedback_div'
  ...
  // 初始化函数
  labcFeedback.init = function(options) {
    if (typeof options.class === 'undefined') {
      console.error('labc-feedback配置项class为必填')
      return
    }
    if (typeof options.back === 'undefined') {
      console.error('labc-feedback配置项back为必填')
      return
    }
    this.options = options
    // 获取所有element
    if (this.options.class instanceof Array)
      for (var i = 0, arr = this.options.class; i < arr.length; i++)
        this.elements.push(document.getElementsByClassName(arr[i]))
    else
    ...
    div.getElementsByTagName('div')[0].onclick = (function(ele, div) {
      ...
      return function() {
        ...
        setTimeout(function() {
          ...
          domtoimage.toPng(element, {imagePlaceholder: '图片加载失败'}).then(function(dataUrl) {
            obj.style.display = 'block'
            labcFeedback.loading.style.display = 'none'
            labcFeedback.options.back(dataUrl)
  ...
  // 启用反馈（添加蒙层）
  labcFeedback.enable = function() {
    if (document.getElementsByClassName(_CLASSNAME_).length > 0) return
    var eles = document.getElementsByClassName(_CLASSNAME_)
    for (var i = 0, arr = this.elements; i < arr.length; i++) {
  ...
  if (typeof module !== 'undefined' && typeof exports === 'object') {
    module.exports = labcFeedback
  } else if (typeof define === 'function' && (define.amd || define.cmd)) {
    define(function() {
      return labcFeedback
    })
  } else {
    $w['labcFeedback'] = labcFeedback
  }
})(window)
